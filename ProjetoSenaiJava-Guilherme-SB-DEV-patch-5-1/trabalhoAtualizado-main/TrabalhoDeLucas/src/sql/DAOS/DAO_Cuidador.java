...
public class DAO_Cuidador {
    ...
    private static String instrucao_delete = "DELETE FROM UsuarioCuidador WHERE IDCuidador = ?;";
        ...
        for(UsrCuidador usrCuidador: consultaCuidador()){
            if(consultaCuidador().get(counterLogin).getCpf().equals(cpf)){
                if( consultaCuidador().get(counterLogin).getSenhaCuidador().equals(senha)){
                    result = true;
                }
        ...
        if(result == true){
            ...
            InterfacePrincipalCuidador inter = new InterfacePrincipalCuidador();
            inter.dispose();
            inter.setVisible(true);
        }else if(result == false){
            JOptionPane.showMessageDialog(null, "cfp ou senha invalidos");
